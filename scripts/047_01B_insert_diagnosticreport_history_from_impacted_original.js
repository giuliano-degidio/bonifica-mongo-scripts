...
(async () => {
  ...
  const historyColl = params.historyColl || "diagnosticreport_history";

  const bulkWrite = params.bulkWrite ?? 1000;
  const logEvery = params.logEvery ?? 1000;

  const historySuffix = params.historySuffix ?? null; // if null => Date.now()
  const exportKeys = params.exportKeys ?? true;
  const exportFileName = params.exportFileName || "diagnosticreport_history_inserted_id.js";
  const exportConstName = params.exportConstName || "DIAGNOSTICREPORT_HISTORY_INSERTED_ID";
  ...
  try {
   // if (!collectionExists(sourceColl)) throw new Error(`Source collection not found: ${sourceColl}`);
    if (!collectionExists(sourceColl)) {
          ...
          if (exportStream) {
            try { exportStream.write("];\n"); } catch (_) {}
            try { exportStream.end(); } catch (_) {}
          }
          return; 
       }	
    ...
    while (cursor.hasNext()) {
      ...
      bulkOps.push({
        replaceOne: { filter: { _id: newDoc._id }, replacement: newDoc, upsert: true }
      });
    ...
    print(JSON.stringify({
      type: "result",
      script: "047_01B_insert_diagnosticreport_history_from_impacted_original.js",
      runId, stepId, dbName,
