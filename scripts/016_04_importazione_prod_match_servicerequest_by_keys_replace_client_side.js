...
async function importOneFile(COLL, FILE, fileIndex, totalFiles, MODE, BATCH, LOG_EVERY_LINES) {
  ...
  log(`FILE_END [${fileIndex}/${totalFiles}]: lines=${totalLines} emptyLines=${emptyLines} parsedDocs=${parsedDocs} bulks=${bulks} opsExecuted=${totalOpsExecuted} inserted=${totalInserted} upserted=${totalUpserted} modified=${totalModified}`);
...
(async () => {
  ...
  const filePrefix = params.filePrefix || "servicerequest_impacted_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "servicerequest_impacted_modified";
  ...
  print(JSON.stringify({
    type: "result",
    script: "016_04_importazione_prod_match_servicerequest_by_keys_replace_client_side.js",
    runId, stepId, dbName,
