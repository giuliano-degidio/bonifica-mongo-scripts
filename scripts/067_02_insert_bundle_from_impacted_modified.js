...
(async () => {
  ...
  const sourceColl = params.sourceColl || "bundle_impacted_modified";
  ...
  try {
    ...
    while (cursor.hasNext()) {
      ...
      if (ops.length >= batch) {
        ...
        log(`BULK_END #${bulks}: ops=${ops.length} inserted=${res.insertedCount || 0} upserted=${res.upsertedCount || 0} modified=${res.modifiedCount || 0} | elapsed=${fmtDuration(Date.now() - bulkStart)}`);
        ops = [];
      }

      if (logEvery > 0 && readDocs % logEvery === 0) {
        log(`PROGRESS: readDocs=${readDocs} bulks=${bulks} opsExecuted=${totalOpsExecuted} | elapsed=${fmtDuration(Date.now() - startMs)}`);
      }
    }
    ...
    log(`END: elapsed=${fmtDuration(Date.now() - startMs)} | TARGET_COLL=${targetColl}`);

    print(JSON.stringify({
      type: "result",
      script: "067_02_insert_bundle_from_impacted_modified.js",
      runId, stepId, dbName,
