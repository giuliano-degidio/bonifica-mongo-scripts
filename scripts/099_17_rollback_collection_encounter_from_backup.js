...
try {
  // ------------------------------------------------------------------------
  // CONTROLLA LE COLLEZIONI MA NON TERMINA LO SCRIPT SU ASSENZE
  // ------------------------------------------------------------------------
  const collectionsAvailable = {
    [TARGET_COLL]: collectionExists(TARGET_COLL),
    [BACKUP_COLL_OLD]: collectionExists(BACKUP_COLL_OLD),
    [WS_MODIFIED_BCK_COLL]: collectionExists(WS_MODIFIED_BCK_COLL),
    [WS_ORIGINAL_BCK_COLL]: collectionExists(WS_ORIGINAL_BCK_COLL)
  };
  const missingCollections = Object.keys(collectionsAvailable).filter(
    name => !collectionsAvailable[name]
  );
  if (missingCollections.length > 0) {
    log(`SKIP: missing required collections: ${missingCollections.join(", ")} -- proseguo comunque con le altre disponibili`);
  }
  ...
  // ========================================================================
  if (collectionsAvailable[TARGET_COLL] && collectionsAvailable[BACKUP_COLL_OLD]) {
    ...
    log(`STEP_1_SKIP: restore encounter from old backup (coll mancanti: ${[BACKUP_COLL_OLD, TARGET_COLL].filter(x => !collectionsAvailable[x]).join(", ")})`);
  }
  ...
  if (sourceExpDir) {
    ...
    if (!oldHistoryIds) {
      log(`STEP_2_HISTORY_DELETE_SKIP: export file not configured o non trovato. path=${oldHistoryExportPath}`);
  ...
  // ========================================================================
  if (collectionsAvailable[TARGET_COLL] && collectionsAvailable[WS_MODIFIED_BCK_COLL]) {
    ...
    log(`STEP_A_SKIP: delete encounter modified (without-status) (coll mancanti: ${[WS_MODIFIED_BCK_COLL, TARGET_COLL].filter(x => !collectionsAvailable[x]).join(", ")})`);
  }

  // ========================================================================
  // STEP B: restore encounter original (without-status) into target (upsert)
  // ========================================================================
  if (collectionsAvailable[TARGET_COLL] && collectionsAvailable[WS_ORIGINAL_BCK_COLL]) {
    ...
    log(`STEP_B_SKIP: restore encounter original (without-status) (coll mancanti: ${[WS_ORIGINAL_BCK_COLL, TARGET_COLL].filter(x => !collectionsAvailable[x]).join(", ")})`);
  }
  ...
  if (sourceExpDir) {
    ...
    if (!wsHistoryIds) {
      log(`STEP_C_HISTORY_DELETE_SKIP: export file not configured o non trovato. path=${wsHistoryExportPath}`);
    } else if (!historyExists) {
  ...
  // ========================================================================
  // END / COUNTS: stampa solo le collezioni trovate
  // ========================================================================
  const targetCountAfter = collectionsAvailable[TARGET_COLL] ? db.getCollection(TARGET_COLL).countDocuments() : 0;
  const historyCountAfter = historyExists ? db.getCollection(HISTORY_COLL).countDocuments() : 0;

  log(
    `COUNTS_AFTER: target_after=${targetCountAfter} | history_after=${historyCountAfter}`
  );
