...
mapFilePath="${logDir}/${mapFileName}"
processedFile="${logDir}/processed.jsonl"
notProcessedFile="${logDir}/not_processed.jsonl"
...
tmpList="$(mktemp)"
trap 'rm -f "$tmpList" "$tmpList.dedup" "$tmpList.worker"' EXIT
...
while IFS= read -r row; do
  ...
  [[ "$processSIO" == "true" && "$left" == Encounter/SIO.* ]] && echo "$left" >> "$tmpList"
  [[ "$processIHUB" == "true" && "$right" == Encounter/IHUB.* ]] && echo "$right" >> "$tmpList"
  [[ "$processHC40_ADT" == "true" && "$right" == Encounter/HC40-ADT.* ]] && echo "$right" >> "$tmpList"
done <<< "$map_lines"
...
[[ "$total" -eq 0 ]] && { log "Nothing to do, exiting."; exit 0; }
...
TOKEN="$(login | tr -d '\r\n')"
if [[ -z "$TOKEN" ]]; then elog "Login empty token"; exit 2; fi
log "Login OK. Token length: ${#TOKEN}"

export BASE_URL TOKEN NOT_PROCESSED_PATH="$notProcessedFile" PROCESSED_PATH="$processedFile" USERNAME PASSWORD
...
one() {
  ...
  curl -sS -D "$tmpHdr" -o "$tmpBody" \
    -H "Authorization: Bearer $TOKEN" -H "Accept: application/fhir+json" \
  ...
  if [[ "$status" != "200" ]]; then
    ...
    jsonl_append "$NOT_PROCESSED_PATH" "$(jq -cn \
      --arg ts "$(ts)" --arg encounterRef "$encRef" --arg encounterId "$encId" \
      --arg type "$type" --arg phase "GET_ADT" --arg httpStatus "$status" \
      --arg diagnostics "$diag" '{ts:$ts, encounterRef:$encounterRef, encounterId:$encounterId, type:$type, phase:$phase, httpStatus:($httpStatus|tonumber?), diagnostics:$diagnostics}')"
  ...
  if [[ "$rt" != "Encounter" ]]; then
    jsonl_append "$NOT_PROCESSED_PATH" "$(jq -cn \
      --arg ts "$(ts)" --arg encounterRef "$encRef" --arg encounterId "$encId" \
      --arg type "$type" --arg phase "GET_ADT_PARSE" \
  ...
  if [[ ! "$putStatus" =~ ^2 ]]; then
    ...
    jsonl_append "$NOT_PROCESSED_PATH" "$(jq -cn \
      --arg ts "$(ts)" --arg encounterRef "$encRef" --arg encounterId "$encId" \
      --arg type "$type" --arg phase "PUT_CDR" --arg httpStatus "$putStatus" \
      --arg diagnostics "$diag2" '{ts:$ts, encounterRef:$encounterRef, encounterId:$encounterId, type:$type, phase:$phase, httpStatus:($httpStatus|tonumber?), diagnostics:$diagnostics}')"
  ...
  jsonl_append "$PROCESSED_PATH" "$(jq -cn \
    --arg ts "$(ts)" --arg encounterRef "$encRef" --arg encounterId "$encId" \
    --arg type "$type" --arg putStatus "$putStatus" \
...
cat "$tmpList.dedup" \
  | xargs -n 1 -P "$numEncounterConcurrent" -I {} "$tmpList.worker" "{}" \
  | awk "$awk_batch"
