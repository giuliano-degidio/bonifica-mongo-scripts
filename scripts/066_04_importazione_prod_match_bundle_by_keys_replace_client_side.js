...
async function importOneFile(COLL, FILE, fileIndex, totalFiles, MODE, BATCH, LOG_EVERY_LINES) {
  ...
  for await (const line of rl) {
    ...
    if (LOG_EVERY_LINES > 0 && totalLines % LOG_EVERY_LINES === 0) {
      log(`PROGRESS file[${fileIndex}/${totalFiles}]: lines=${totalLines} parsed=${parsedDocs} bulks=${bulks} opsExecuted=${totalOpsExecuted}`);
    }
  }
...
(async () => {
  ...
  const filePrefix = params.filePrefix || "bundle_impacted_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "bundle_impacted_modified";
  ...
  print(JSON.stringify({
    type: "result",
    script: "066_04_importazione_prod_match_bundle_by_keys_replace_client_side.js",
    runId, stepId, dbName,
