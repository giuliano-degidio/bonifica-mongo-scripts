// Same importer as 006_04, just different default prefix/coll
const fs = require("fs");
...
async function importOneFile(COLL, FILE, fileIndex, totalFiles, MODE, BATCH, LOG_EVERY_LINES) {
  ...
  log(`FILE_END [${fileIndex}/${totalFiles}]`);
...
(async () => {
  ...
  const filePrefix = params.filePrefix || "documentreference_impacted_original_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "documentreference_impacted_original";
  ...
  print(JSON.stringify({
    type: "result",
    script: "006_04B_importazione_prod_match_documentreference_by_keys_original.js",
    runId, stepId, dbName,
