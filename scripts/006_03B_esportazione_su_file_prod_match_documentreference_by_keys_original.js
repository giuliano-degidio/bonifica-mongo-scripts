...
// Estrae solo la parte sinistra (Encounter/SIO...) dalle coppie "left#right"
...
(function main() {
  ...
  const outBase = params.outBase || "documentreference_impacted_original";
  ...
  // --- logger ---
  const __logStream = logToFile
    ? fs.createWriteStream(logPath, { encoding: "utf8", flags: logFileFlags })
    : null;
  ...
  // summary per output finale (parte 3/3)
  let summary = {
    totalPairs: 0,
    chunkSize: 0,
    chunkCount: 0,
    ...
    totalExportedDocs: 0,
    chunks: [] // { chunkNo, from, to, mapRows, keys, badRows, outPath, statePath, exported, completed, skipped }
  };
  ...
  // ---- try globale: continua in PARTE 2/3 ----
    // ---- try globale: continua in PARTE 2/3 ----
  try {
    load(mapPath); // defines mapConstName
    ...
    // stato globale (quali chunk completati)
    const globalState = readJsonIfExists(globalStatePath, {
      completedChunks: [], // array di numeri 1-based
      updated_at: null
    });
      ...
      while (true) {
        ...
        for (const doc of docs) {
          ...
          if (logEvery > 0 && exported % logEvery === 0) {
            ...
            logLine(
              `PROGRESS: chunk#${chunkNo} ${new Date(nowMs).toISOString()} | exported=${exported} | lastId=${lastId} | elapsed=${fmtDuration(nowMs - startMs)}`
            );
          }
        }
  ...
  print(
    JSON.stringify({
      type: "result",
      script: "006_03B_esportazione_su_file_prod_match_documentreference_by_keys_original.js",
      runId,
      stepId,
      dbName,
