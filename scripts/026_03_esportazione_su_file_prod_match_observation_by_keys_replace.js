...
// ====== DEBUG ======
const DEBUG = false;
const DEBUG_DOC_ID = ""; // es: "Observation/DNLAB.OBS1-40900038-202403040800-10-0-1"
...
function applyReplacementsToDocument(doc, replaceFn, debugCollector) {
  ...
  // Observation: resource.encounter reference (object OR array)
  try {
...
(function main() {
  ...
  const outBase = params.outBase || "observation_impacted";
  ...
  // continue in part 2/3
    ...
    for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
      ...
      while (true) {
        ...
        const cursor = db.observation.find(q).sort({ _id: 1 }).limit(batchSize);
        if (!DEBUG) {
  ...
  print(
    JSON.stringify({
      type: "result",
      script: "026_03_esportazione_su_file_prod_match_observation_by_keys_replace.js",
      runId,
      stepId,
      dbName,
