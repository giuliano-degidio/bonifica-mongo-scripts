 ...
 * temp/017_17_rollback_collection_servicerequest_from_backup.js
 *
 * Standard:
...
function deleteHistoryByIds(ids, dryRun, startMs) {
  if (!ids || ids.length === 0) {
...
const historyExportFile = params.historyExportFile || "servicerequest_history_inserted_id.js";
const historyExportFilePath = sourceExpDir ? path.join(String(sourceExpDir), String(historyExportFile)) : null;

log(`START: rollback servicerequest | runId=${runId} env=${env} | sourceRunId=${sourceRunId} sourceExpDir=${sourceExpDir} | TARGET_COLL=${TARGET_COLL} | MODIFIED_BCK_COLL=${MODIFIED_BCK_COLL} | ORIGINAL_BCK_COLL=${ORIGINAL_BCK_COLL} | DRY_RUN=${dryRun}`);
...
try {
  ...
  if (!sourceExpDir) {
    ...
    else deleteHistoryByIds(historyIds, dryRun, startMs);
  }
