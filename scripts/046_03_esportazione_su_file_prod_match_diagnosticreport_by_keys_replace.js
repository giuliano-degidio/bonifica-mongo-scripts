...
const DEBUG_DOC_ID = ""; // es: "diagnosticreport/bfaf34e0-f860-4f12-9e6a-ec1555996ce1"
const DEBUG_NO_WRITE = false; // se true, non scrive JSONL
const DEBUG_PRINT_MAP_FOR_KEY = ""; // es: "Encounter/SIO.08008224802062R"

// ====== CHUNK MAPPA DEFAULT ======
const DEFAULT_MAP_CHUNK_SIZE = 1000; // numero righe mappa per chunk (0 o <=0 => tutto in un colpo)
...
// ====== QUERY DEFAULT ======
//const DEFAULT_MATCH_NAME = "encounter:context.encounter.reference";
const DEFAULT_MATCH_NAME = "encounter:encounter.reference";
const DEFAULT_HINT_NAME = "attribute_search.name_1_attribute_search.value_1__id_1"; // best-effort
...
(function main() {
  ...
  // ====== PARAMETRI STANDARDIZZATI (stile B) ======
  const outDir = path.join(expDir, String(runId || "no-runid"));
  fs.mkdirSync(outDir, { recursive: true });

  const outBase = params.outBase || "diagnosticreport_impacted";
  ...
  function buildMapsFromPairs(pairs) {
    ...
    for (const row of pairs) {
      ...
      // ====== TOKEN MAP: SIO.<token> -> IHUB.<token> OR HC40-ADT.<token> ======
      // NB: collisioni sullo stesso leftTok => last wins (overwrite).
      ...
      if (idxS >= 0 && (idxI >= 0 || idxH >= 0)) {
        ...
        let rightTok = "";
        if (idxI >= 0) rightTok = rightAfter.slice(idxI + 5); // dopo "IHUB."
        else rightTok = rightAfter.slice(idxH + 9); // dopo "HC40-ADT."
        ...
        let rightTok = "";
        if (idxIL >= 0) rightTok = rightAfter.slice(idxIL + 5);
        else rightTok = rightAfter.slice(idxHL + 9);
  ...
  function applyReplacementsToDocument(doc, replaceFn, debugCollector) {
    ...
    // resource.context.encounter[*].reference
	/* OLD
    try {
      const enc = doc?.resource?.context?.encounter;
      if (Array.isArray(enc)) {
        for (let i = 0; i < enc.length; i++) {
          const e = enc[i];
          if (e && typeof e === "object") rep(e, "reference", `resource.context.encounter[${i}].reference`);
        }
      }
    } catch (_) {}
	*/
	//new
  try {
    // old const enc = doc?.resource?.context?.encounter;
    const enc = doc?.resource?.encounter;
    // new resource.encounter

    // CASO 1: encounter è un OGGETTO { reference: "..." }
    if (enc && typeof enc === "object" && !Array.isArray(enc)) {
      rep(enc, "reference", "resource.encounter.reference");
    }

    // CASO 2 (fallback/robustezza): encounter è un ARRAY [{ reference: "..." }, ...]
    if (Array.isArray(enc)) {
      for (let i = 0; i < enc.length; i++) {
        const e = enc[i];
...
// fine new
  ...
  // ---- inizio try globale: nella Parte 2/3 ----
    // ---- inizio try globale: nella Parte 2/3 ----
  let summary = {
    totalPairs: 0,
    chunkSize: 0,
    ...
    chunks: [] // { chunkNo, from, to, mapRows, outPath, statePath, exported, replDirect, replPipeSuffix, completed }
  };

  try {
    // MAP LOAD: file JS che definisce constName (default: SIO_IHUB_HC40_ADT_KEYS)
    load(mapPath);

    const pairs = globalThis[mapConstName];
    if (!Array.isArray(pairs) || pairs.length === 0) {
      throw new Error(`${mapConstName} non è un array valido o è vuoto. Controlla: ${mapPath}`);
    }
    ...
    // stato globale: chunk completati
    const globalState = readJsonIfExists(globalStatePath, {
      completedChunks: [], // array numeri 1-based
      updated_at: null
    });
    ...
    for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
      const chunkNo = chunkIndex + 1; // 1-based
      ...
      if (completedSet.has(chunkNo) && !DEBUG) {
        ...
        summary.chunks.push({
          ...
          from,
          to: to - 1,
          mapRows: chunkPairs.length,
          outPath: outPathChunk,
          statePath: statePathChunk,
          exported: 0,
          replDirect: 0,
          replPipeSuffix: 0,
          completed: true,
      ...
      const { maps, sioKeysFullUpper, badRows } = buildMapsFromPairs(chunkPairs);
      const replaceExactFn = makeReplaceExactFn(maps);
      const replaceFn = makeReplaceFnWithPipeSuffix(replaceExactFn);

      logLine(`CHUNK_MAP: #${chunkNo} | pairs=${chunkPairs.length} | matchKeys=${sioKeysFullUpper.length} | badRows=${badRows}`);
      ...
      if (chunkState.completed && !DEBUG) {
        // aggiorno global state comunque
      ...
      while (true) {
        ...
        const cursor = db.diagnosticreport
          .find(q)
          .sort({ _id: 1 })
          .limit(batchSize);
        ...
        for (const doc of docs) {
          ...
          if (logEvery > 0 && exported % logEvery === 0) {
            ...
            logLine(
              `PROGRESS: chunk#${chunkNo} ${new Date(nowMs).toISOString()} | exported=${exported} | lastId=${lastId} | repl_direct=${totalDirect + batchDirect} | repl_pipeSuffix=${totalPipe + batchPipe} | elapsed=${fmtDuration(nowMs - startMs)}`
            );
          }
        }
        ...
        // checkpoint per chunk dopo ogni batch
        if (!DEBUG) {
      ...
      summary.chunks.push({
        ...
        statePath: statePathChunk,
        exported,
        replDirect: totalDirect,
        replPipeSuffix: totalPipe,
        completed: !DEBUG,
        skipped: false
      });
    ...
    // Parte 3/3: chiusura stream + JSON result
  } catch (e) {
  ...
  // Nota: chiusura stream + JSON result in Parte 3/3
    // Nota: chiusura stream + JSON result in Parte 3/3

  // Chiudi log file stream
  try { if (__logStream) __logStream.end(); } catch (_) {}

  // Machine-friendly output
  // NB: lo stampo DOPO aver chiuso lo stream log, così eventuali flush non mescolano output
  print(
    JSON.stringify({
      type: "result",
      script: "046_03_esportazione_su_file_prod_match_diagnosticreport_by_keys_replace.js",
      runId,
      stepId,
      dbName,
