...
// ====== DEBUG ======
const DEBUG = false;
const DEBUG_DOC_ID = ""; // es: "bundle/...." oppure altro _id
...
function buildMapsFromPairs(pairs) {
  ...
  for (const row of pairs) {
    ...
    // Bundle match: Encounter.id contiene "Encounter/SIO...."
...
// ---------- apply replacements (bundle-specific) ----------
function applyReplacementsToDocument(doc, replaceFn, debugCollector) {
  ...
  function rep(obj, key, pth) {
    ...
    // safety: don't touch EJSON numeric wrappers like timestamp.$numberLong
    if (pth === 'timestamp["$numberLong"]' || pth === "timestamp.$numberLong") return;
      ...
      for (let i = 0; i < as.length; i++) {
        ...
        if (Array.isArray(joins)) {
          for (let j = 0; j < joins.length; j++) {
            const jdoc = joins[j];
            if (!jdoc || typeof jdoc !== "object") continue;

            rep(jdoc, "_id", `attribute_search[${i}].join[${j}]._id`);

            const jas = jdoc.attribute_search;
  ...
  // 3) resource.entry[*].resource.context.encounter[*].reference
  try {
    const entries = doc?.resource?.entry;
    if (Array.isArray(entries)) {
      for (let ei = 0; ei < entries.length; ei++) {
        const entry = entries[ei];
        if (!entry || typeof entry !== "object") continue;

        const encArr = entry?.resource?.context?.encounter;
        if (Array.isArray(encArr)) {
          for (let i = 0; i < encArr.length; i++) {
            const e = encArr[i];
            if (e && typeof e === "object") {
              rep(e, "reference", `resource.entry[${ei}].resource.context.encounter[${i}].reference`);
            }
          }
        }
      }
  ...
  // 4) resource.entry[*].resource.id
  try {
    const entries = doc?.resource?.entry;
    if (Array.isArray(entries)) {
      for (let ei = 0; ei < entries.length; ei++) {
        const res = entries[ei]?.resource;
        if (res && typeof res === "object") rep(res, "id", `resource.entry[${ei}].resource.id`);
      }
    }
  } catch (_) {}

  // 5) resource.entry[*].resource.identifier[*].value
  try {
    const entries = doc?.resource?.entry;
    if (Array.isArray(entries)) {
      for (let ei = 0; ei < entries.length; ei++) {
        const ids = entries[ei]?.resource?.identifier;
        if (!Array.isArray(ids)) continue;

        for (let i = 0; i < ids.length; i++) {
          const idObj = ids[i];
          if (idObj && typeof idObj === "object") rep(idObj, "value", `resource.entry[${ei}].resource.identifier[${i}].value`);
        }
      }
    }
  } catch (_) {}

  // 6) DocumentReference.context.encounter[*].reference (se presente)
  try {
    const drEnc = doc?.DocumentReference?.context?.encounter;
    if (Array.isArray(drEnc)) {
      for (let i = 0; i < drEnc.length; i++) {
        const e = drEnc[i];
        if (e && typeof e === "object") rep(e, "reference", `DocumentReference.context.encounter[${i}].reference`);
      }
    }
  } catch (_) {}

  // 7) Encounter.id (top-level object)
  try {
    const enc = doc?.Encounter;
    if (enc && typeof enc === "object" && !Array.isArray(enc)) rep(enc, "id", "Encounter.id");
  } catch (_) {}
...
(function main() {
  ...
  const outBase = params.outBase || "bundle_impacted";
  ...
  logLine(`CONFIG: MATCH_NAME=${matchName} | MAP_CHUNK_SIZE=${mapChunkSize} | BATCH_SIZE=${batchSize} | LOG_EVERY=${logEvery}`);
  ...
  // continue in part 2/3
    ...
    for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
      ...
      logLine(`CHUNK_START: #${chunkNo}/${chunkCount} | map_from=${from} | map_to=${to - 1} | map_rows=${chunkPairsOriginal.length} | outPath=${outPathChunk}`);
      ...
      const replaceFn = makeReplaceFnWithPipeSuffix(makeReplaceExactFn(maps));
      ...
      while (true) {
        ...
        for (const doc of docs) {
          ...
          if (DEBUG) {
            ...
            for (const ch of debugChanges) logLine(`  - [${ch.mode}] ${ch.path}: "${ch.before}" -> "${ch.after}"`);
          }

          // bundle: salviamo EJSON non-relaxed
          if (out) out.write(EJSON.stringify(doc, { relaxed: false }) + "\n");
  ...
  print(
    JSON.stringify({
      type: "result",
      script: "066_03_esportazione_su_file_prod_match_bundle_by_keys_replace.js",
      runId,
      stepId,
      dbName,
