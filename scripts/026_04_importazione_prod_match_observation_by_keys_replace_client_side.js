// Standard importer (stile B): legge da expDir/runId
const fs = require("fs");
...
async function importOneFile(COLL, FILE, fileIndex, totalFiles, MODE, BATCH, LOG_EVERY_LINES) {
  ...
  for await (const line of rl) {
    ...
    if (ops.length >= BATCH) {
      ...
      totalInserted += inserted; totalUpserted += upserted; totalModified += modified;
  ...
  if (ops.length > 0) {
    ...
    log(`BULK_END file[${fileIndex}/${totalFiles}] #${bulks}: ops=${ops.length} inserted=${res.insertedCount || 0} upserted=${res.upsertedCount || 0} modified=${res.modifiedCount || 0} | elapsed=${fmtDuration(Date.now() - bulkStart)}`);
  }
...
(async () => {
  ...
  const filePrefix = params.filePrefix || "observation_impacted_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "observation_impacted_modified";
  ...
  print(JSON.stringify({
    type: "result",
    script: "026_04_importazione_prod_match_observation_by_keys_replace_client_side.js",
    runId, stepId, dbName,
