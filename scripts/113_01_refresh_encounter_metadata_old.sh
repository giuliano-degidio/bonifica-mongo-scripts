...
# La pipeline run_pipeline.sh genera un file JSON context_step_<id>.json per ogni step
...
# Funzione compatibile per estrarre valori dal context (jq)
...
# Altri path di log/output
logDir="${expDir}"
mkdir -p "$logDir"
mapFilePath="${expDir}/${mapFileName}"
...
# Output finale JSON - per tracciamento standard pipeline
