...
(async () => {
  ...
  const historyColl = params.historyColl || "servicerequest_history";

  const bulkWrite = params.bulkWrite ?? 1000;
  const logEvery = params.logEvery ?? 1000;

  const historySuffix = params.historySuffix ?? null; // null => Date.now()
  const exportKeys = params.exportKeys ?? true;
  const exportFileName = params.exportFileName || "servicerequest_history_inserted_id.js";
  const exportConstName = params.exportConstName || "SERVICEREQUEST_HISTORY_INSERTED_ID";
  ...
  try {
    ...
    print(JSON.stringify({
      type: "result",
      script: "017_01B_insert_servicerequest_history_from_impacted_original.js",
      runId, stepId, dbName,
