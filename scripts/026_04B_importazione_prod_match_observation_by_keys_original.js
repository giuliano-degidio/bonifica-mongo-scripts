// Standard importer (stile B): legge da expDir/runId (original chunks)
const fs = require("fs");
...
(async () => {
  ...
  const filePrefix = params.filePrefix || "observation_impacted_original_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "observation_impacted_original";
  ...
  print(JSON.stringify({
    type: "result",
    script: "026_04B_importazione_prod_match_observation_by_keys_original.js",
    runId, stepId, dbName,
