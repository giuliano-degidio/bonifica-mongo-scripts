...
(async () => {
  ...
  const filePrefix = params.filePrefix || "documentreference_impacted_chunk";
  const fileSuffix = params.fileSuffix || ".jsonl";
  const maxFiles = params.maxFiles ?? 0;

  const coll = params.collection || "documentreference_impacted_modified";
  ...
  print(
    JSON.stringify({
      type: "result",
      script: "006_04_importazione_prod_match_documentreference_by_keys_replace_client_side.js",
      runId,
      stepId,
      dbName,
      inDir,
