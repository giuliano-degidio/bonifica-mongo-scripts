 ...
 * temp/047_17_rollback_collection_diagnosticreport_from_backup.js
 *
 * Standard:
...
// ====== COLLECTIONS ======
const MODIFIED_BCK_COLL = "bonifica_diagnosticreport_inserted_bck";
const ORIGINAL_BCK_COLL = "bonifica_diagnosticreport_deleted_bck";
const TARGET_COLL = "diagnosticreport";

// OPTIONAL history rollback
const HISTORY_COLL = "diagnosticreport_history";
const HISTORY_EXPORT_CONST_NAME = "DIAGNOSTICREPORT_HISTORY_INSERTED_ID";
...
const historyExportFile = params.historyExportFile || "diagnosticreport_history_inserted_id.js";
const historyExportFilePath = sourceExpDir ? path.join(String(sourceExpDir), String(historyExportFile)) : null;

log(`START: rollback diagnosticreport | runId=${runId} env=${env} | sourceRunId=${sourceRunId} sourceExpDir=${sourceExpDir} | TARGET_COLL=${TARGET_COLL} | MODIFIED_BCK_COLL=${MODIFIED_BCK_COLL} | ORIGINAL_BCK_COLL=${ORIGINAL_BCK_COLL} | DRY_RUN=${dryRun}`);
...
try {
  ...
  log(`STEP_A_END: readModifiedIds=${readModifiedIds} deleteOpsPrepared=${deleteOpsPrepared} deletedTotal=${deletedTotal} delBulks=${delBulks} | elapsed=${fmtDuration(Date.now() - stepAStart)}`);
  ...
  while (originalCursor.hasNext()) {
    ...
    if (doc == null || typeof doc !== "object" || Array.isArray(doc)) {
      throw new Error(`Invalid doc read from ${ORIGINAL_BCK_COLL} (readOriginalDocs=${readOriginalDocs})`);
    }
    if (doc._id === undefined) {
      throw new Error(`Doc without _id read from ${ORIGINAL_BCK_COLL} (readOriginalDocs=${readOriginalDocs})`);
    }
    ...
    if (upOps.length >= BULK_UPSERT) {
      ...
      if (LOG_EVERY_UPSERTS > 0 && upsertOpsPrepared % LOG_EVERY_UPSERTS === 0) {
        log(`STEP_B_PROGRESS: prepared=${upsertOpsPrepared} readOriginalDocs=${readOriginalDocs} upBulks=${upBulks} inserted=${upInserted} upserted=${upUpserted} modified=${upModified} | elapsed=${fmtDuration(Date.now() - stepBStart)}`);
      }
  ...
  log(`STEP_B_END: readOriginalDocs=${readOriginalDocs} upsertOpsPrepared=${upsertOpsPrepared} upBulks=${upBulks} inserted=${upInserted} upserted=${upUpserted} modified=${upModified} | elapsed=${fmtDuration(Date.now() - stepBStart)}`);
  ...
  if (!sourceExpDir) {
    ...
    if (!historyIds) {
      log(`STEP_C_SKIP: history export file not configured or not found. path=${historyExportFilePath}`);
    } else {
      deleteHistoryByIds(historyIds, dryRun);
    }
  }
