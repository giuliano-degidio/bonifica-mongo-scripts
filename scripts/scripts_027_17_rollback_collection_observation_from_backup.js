 ...
 * temp/027_17_rollback_collection_observation_from_backup.js
 *
 * Standard:
...
const historyExportFile = params.historyExportFile || "observation_history_inserted_id.js";
const historyExportFilePath = sourceExpDir ? path.join(String(sourceExpDir), String(historyExportFile)) : null;

log(`START: rollback observation | runId=${runId} env=${env} | sourceRunId=${sourceRunId} sourceExpDir=${sourceExpDir} | TARGET_COLL=${TARGET_COLL} | MODIFIED_BCK_COLL=${MODIFIED_BCK_COLL} | ORIGINAL_BCK_COLL=${ORIGINAL_BCK_COLL} | DRY_RUN=${dryRun}`);
