...
--quiet ^
--file "C:\Appo09\Mongo_Prod_EXP\027_17_rollback_collection_observation_from_backup.js"
*/
/* PRODUZIONE/TEST
...
/data/mongosh/bin/mongosh "mongodb://root:password@mongo-rs-1.mongo-rs-svc.mongodb.svc.cluster.local:27017/hc40-index-bck?authSource=admin&directConnection=true&readPreference=primary" \
  --quiet \
  --file "/data/Mongo_Sh_Script/027_17_rollback_collection_observation_from_backup.js"

  spostare il file da windows a ubuntu kubernate:
 Get-Content -Raw "C:\Users\giuldegi\OneDrive - Engineering Ingegneria Informatica S.p.A\Desktop\ENG\SANITA\Romagna\CDR-Mongo\Svil\BONIFICA_CDR_TEST\027_17_rollback_collection_observation_from_backup.js" |
kubectl -n ellipse-index exec -i ubuntu-mongosync-6845564564-zvnn9 -- sh -c 'cat > /data/Mongo_Sh_Script/027_17_rollback_collection_observation_from_backup.js'
...
*/
/**
 * 027_17_rollback_collection_observation_from_backup.js
 *
 * Rollback della bonifica observation:
 * - Delete da observation degli _id presenti in bonifica_observation_inserted_bck
 * - Restore (upsert) in observation dei doc in bonifica_observation_deleted_bck
 * - (OPZIONALE) ELIMINA da observation_history i record inseriti dal passo
 *   027_01B_insert_observation_history_from_impacted_original.js, utilizzando gli _id presenti
 *   nel file export generato (es. observation_history_inserted_id_....js).
 *   Se il file non esiste / non è configurato, questo passo viene SKIPPATO (guardia di esistenza file).
 */

const fs = require("fs");

const MODIFIED_BCK_COLL = "bonifica_observation_inserted_bck";
const ORIGINAL_BCK_COLL = "bonifica_observation_deleted_bck";
const TARGET_COLL = "observation";

// history cleanup
const HISTORY_COLL = "observation_history";
//const HISTORY_EXPORT_FILE_PATH = "C:\\Appo09\\Mongo_Prod_EXP\\EXP\\observation_history_inserted_id.js"; // <-- aggiornare al file reale
const HISTORY_EXPORT_FILE_PATH = "/data/Mongo_Sh_Script/EXP/observation_history_inserted_id.js"; // <-- aggiornare al file reale
const HISTORY_EXPORT_CONST_NAME = "OBSERVATION_HISTORY_INSERTED_ID";
...
function deleteHistoryByIds(ids, startMs) {
  ...
  log(`STEP_C_START: delete from ${HISTORY_COLL} using export file ids | ids=${ids.length} | DRY_RUN=${DRY_RUN}`);
  let ops = [];
  let processed = 0;
  let bulks = 0;
  let deletedTotal = 0;
...
log(`START: rollback observation | TARGET_COLL=${TARGET_COLL} | DRY_RUN=${DRY_RUN} | HISTORY_EXPORT_FILE_PATH=${HISTORY_EXPORT_FILE_PATH}`);
...
try {
  ...
  log("END: rollback observation (DRY_RUN=" + DRY_RUN + ")");
} catch (e) {
