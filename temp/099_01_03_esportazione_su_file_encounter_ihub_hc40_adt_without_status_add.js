...
mongosh "mongodb://giuldegi:bitbros@192.168.248.135:27017/romagna?authSource=admin" ^
  --quiet ^
  --file "C:\Appo09\Mongo_Prod_EXP\099_01_03_esportazione_su_file_encounter_ihub_hc40_adt_without_status_add.js"
*/
/* PRODUZIONE/TEST
...
--quiet ^
--file "C:\Appo10\Mongo_Prod_EXP\099_01_03_esportazione_su_file_encounter_ihub_hc40_adt_without_status_add.js"
*/
...
/data/mongosh/bin/mongosh "mongodb://root:password@mongo-rs-1.mongo-rs-svc.mongodb.svc.cluster.local:27017/hc40-index-bonifica?authSource=admin&directConnection=true&readPreference=primary" \
  --quiet \
  --file "/data/Mongo_Sh_Script/099_01_03_esportazione_su_file_encounter_ihub_hc40_adt_without_status_add.js"

Spostare il file da Windows a Ubuntu Kubernetes:
/*
Get-Content -Raw "C:\Appo10\Mongo_Prod_EXP\099_01_03_esportazione_su_file_encounter_ihub_hc40_adt_without_status_add.js" |
kubectl -n ellipse-index exec -i ubuntu-mongosync-6845564564-zvnn9 -- sh -c 'cat > /data/Mongo_Sh_Script/099_01_03_esportazione_su_file_encounter_ihub_hc40_adt_without_status_add.js'
*/
/*

log:
START: 2026-04-23T11:09:05.584Z | DEBUG=false | DEBUG_DOC_ID=(none) | MAP_PATH=C:\Appo10\Mongo_Prod_EXP\EXP\sio_ihub_hc40_adt_keys_map.js
LOG: logPath=C:\Appo10\Mongo_Prod_EXP\EXP\encounter_without_status_add.log | LOG_TO_CONSOLE=true | LOG_FILE_FLAGS=a
CONFIG: MAP_CHUNK_SIZE=1000 | BATCH_SIZE=2000 | LOG_EVERY=1000 | DEBUG_NO_WRITE=false
STATE: GLOBAL_STATE_PATH=C:\Appo10\Mongo_Prod_EXP\EXP/encounter_without_status_add.chunks.state.json
MAP_TOTAL: pairs=36 | chunkSize=1000 | chunkCount=1
GLOBAL_STATE: completedChunks=(none)
CHUNK_START: #1/1 | map_from=0 | map_to=35 | map_rows=36 | outPath=C:\Appo10\Mongo_Prod_EXP\EXP/encounter_without_status_add_chunk0001.jsonl | statePath=C:\Appo10\Mongo_Prod_EXP\EXP/encounter_without_status_add_chunk0001.state.json
...
CHUNK_END: #1/1 | exported=19 | outPath=C:\Appo10\Mongo_Prod_EXP\EXP/encounter_without_status_add_chunk0001.jsonl | completed=true
END: 2026-04-23T11:09:09.455Z | elapsed=0h 0m 3s

*/

// SCOPO
// -----
// - Legge la mappa sio_ihub_hc40_adt_keys_map.js (array SIO_IHUB_HC40_ADT_KEYS con righe "left#right").
// - Prende TUTTI gli _id encounter di DESTRA (right) => encounter IHUB / HC40-ADT.
// - Per ognuno controlla se in (root)->attribute_search (array) esiste un oggetto con name == "status:status".
// - Se NON esiste:
//     A) aggiunge/forza doc.resource.status = "finished"
//     B) aggiunge in doc.attribute_search l'oggetto { name:"status:status", value:"finished" }
//   ed esporta il documento intero in JSONL su file encounter_without_status_add_chunkXXXX.jsonl
//
// - Lavora a chunk sulla MAPPA (MAP_CHUNK_SIZE) come 006_03 e implementa resume:
//     - stato globale: encounter_without_status_add.chunks.state.json (quali chunk completati)
//     - stato per chunk: encounter_without_status_add_chunkXXXX.state.json (lastId/exported)
//
// NOTE
// ----
// - Se attribute_search non esiste o non è un array: viene creato come array.
// - Prima di rilanciare da zero: cancellare i file encounter_without_status_add* in EXP.

const fs = require("fs");

// ====== DEBUG ======
const DEBUG = false;
// se valorizzato, processa solo quel documento (utile per debug)
const DEBUG_DOC_ID = ""; // es: "Encounter/...."
...
// Windows example:
const logPath = "C:\\Appo10\\Mongo_Prod_EXP\\EXP\\encounter_without_status_add.log";
//const logPath = "/data/Mongo_Sh_Script/EXP/encounter_without_status_add.log";
const LOG_FILE_FLAGS = "a"; // "w" sovrascrive, "a" append

// ====== INPUT MAPPA ======
// Windows example:
 const MAP_PATH = "C:\\Appo10\\Mongo_Prod_EXP\\EXP\\sio_ihub_hc40_adt_keys_map.js";
//const MAP_PATH = "/data/Mongo_Sh_Script/EXP/sio_ihub_hc40_adt_keys_map.js"; // defines SIO_IHUB_HC40_ADT_KEYS

// ====== OUTPUT ======
//const OUT_DIR = "/data/Mongo_Sh_Script/EXP";
// Windows example:
 const OUT_DIR = "C:\\Appo10\\Mongo_Prod_EXP\\EXP";
const OUT_BASE = "encounter_without_status_add";
const OUT_EXT = ".jsonl";

// ====== CHUNK MAPPA ======
const MAP_CHUNK_SIZE = 1000; // numero righe mappa per chunk (0 o <=0 => tutto in un colpo)

// ====== RESUME (globale + per chunk) ======
...
// Estrae gli id encounter di DESTRA (dopo #) dalle coppie "left#right"
...
function addStatusFinished(doc) {
  // A) resource.status = "finished"
  if (!doc.resource || typeof doc.resource !== "object" || Array.isArray(doc.resource)) {
    // da tua nota resource esiste sicuramente, ma gestiamo safe
    doc.resource = {};
  }
  doc.resource.status = "finished";

  // B) attribute_search push
...
logLine(`LOG: logPath=${logPath} | LOG_TO_CONSOLE=${LOG_TO_CONSOLE} | LOG_FILE_FLAGS=${LOG_FILE_FLAGS}`);
...
try {
  ...
  load(MAP_PATH); // defines SIO_IHUB_HC40_ADT_KEYS
  ...
  const globalState = readJsonIfExists(GLOBAL_STATE_PATH, {
    completedChunks: [], // array of 1-based chunk numbers
    updated_at: null,
  });
  ...
  for (let chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
    ...
    // resume per chunk (scorri per _id)
    ...
    while (true) {
      ...
      for (const doc of docs) {
        ...
        if (!has) {
          // aggiungi status e attribute_search object
